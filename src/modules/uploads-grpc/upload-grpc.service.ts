...
interface uploadServiceClient {
  ...
  GetUserImage(data: GetUserImageRequest): Observable<GetUserImageResponse>
}
...
export class UploadGrpcService implements OnModuleInit {
  ...
  async getUserImage(data: GetUserImageRequest): Promise<GetUserImageResponse> {
    return this.grpc.call(this.uploadServiceName, this.uploads.GetUserImage(data));
  }
}
